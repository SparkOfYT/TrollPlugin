			...
			if (player.isOp() || player.hasPermission("troll.use.burn")) {
				if (target != null) {
					if (!Var.BURN.contains(target)) {
						Var.BURN.add(target);
						player.sendMessage(Var.PREFIX + "§6§l" + target.getName() + " §abrennt nun!");
					} else {
						Var.BURN.remove(target);
						player.sendMessage(Var.PREFIX + "§6§l" + target.getName() + " §cbrennt nun nicht mehr!");
					}
